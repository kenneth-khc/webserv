...
void	CGI::generateEnv(void)
{
	...
	String				remoteAddr = server.clients.find(server.readyEvents[0].data.fd)->second.getIPAddr();
	String				contentLength = request["Content-Length"].value;

	stream << server.portNum;
	...
	const String	metaVariables[NUM_OF_META_VARIABLES] = {
		...
		"X_UPLOADS_DIR=" + server.uploadsDir
	};
...
void	CGI::execute(void) {
	...
	if (contentLength != "" && contentLength != "0") {
		write(this->dataSend[1], request.messageBody.c_str(), request.find< Optional<String::size_type> >("Content-Length").value);
	}
	close(this->dataSend[0]);
	close(this->dataSend[1]);
	close(this->dataRecv[1]);
